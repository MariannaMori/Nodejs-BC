...
function initLayerFunctions(baseDir, config) {
    ...
    const storageConfig = required(path.join(storageFolder, config.storageConfig));
    ...
    async function getFromStorage(key, value) {
        return (await readStorage(storageFile)).find(resultObject => resultObject[key]==value) || null;
    }
